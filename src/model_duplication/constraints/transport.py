    ...
    def apply(self, model: Model, phases: Phases):
        ...
        for transport in self.transports:
            ...
            try:
                ...
                raise KeyError(f"In {transport}, phase {transport.source} was "
                               f"used. However, the phase {transport.source} is"
                               f" unknown to the Phases object.") from e
            try:
                destination = phases.phases.get_by_id(transport.destination)
            except KeyError as e:
                raise KeyError(f"In {transport}, phase {transport.destination} was "
            ...
            try:
                ...
                raise KeyError(
                    f"Metabolite "
                    f"{transport.metabolite_id + '_' + transport.destination} was "
                    f"not found in model with id {model.id}. Associated "
                    f"transport cannot be created. Check that metabolite "
                    f"{transport.metabolite_id} exists in the model associated "
            ...
            logging.info(f"The previously stated reactions (n= {len(reactions2add)}) were added to the model.")
